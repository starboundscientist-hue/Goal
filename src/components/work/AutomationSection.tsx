...
export function AutomationSection() {
  ...
  return (
    <div className="mt-8">
      <h3 className="text-xs uppercase tracking-wider text-muted-foreground font-medium mb-3">Automation</h3>
      ...
      {showForm ? (
        <div className="bg-surface-card border border-surface-border rounded-lg p-3 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-32 bg-surface-base border border-surface-border rounded px-2 py-1.5 text-sm text-foreground outline-none"
            />
            <span className="text-xs text-muted-foreground">hours/week saved</span>
          </div>
        ...
        <button onClick={() => setShowForm(true)} className="text-xs text-muted-foreground hover:text-foreground border border-surface-border rounded-md px-3 py-1.5">
          + Log automation
        </button>
      )}
    </div>
