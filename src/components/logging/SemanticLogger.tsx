...
export function SemanticLogger() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] animate-fade-in">
      ...
      <div className="relative bg-surface-card/80 backdrop-blur-2xl border border-surface-border/60 rounded-2xl max-w-lg w-full mx-4 shadow-2xl conic-border grain overflow-hidden">
        <div className="p-5">
          <h2 className="text-sm font-medium text-foreground/90 mb-3">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground/50">
                  {llmOnline ? 'AI online \u00b7 auto-tagging enabled' : 'AI offline \u00b7 will use manual form'}
                </span>
                <button onClick={handleSubmit} className="text-xs text-white bg-blue-600/90 hover:bg-blue-500 px-3 py-1.5 rounded-lg transition-colors">
                  Parse {'\u21b5'}
                </button>
              </div>
            </div>
          ...
          {state === 'preview' && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-muted-foreground/50 mb-1 block">Cluster</label>
                  <select
                    value={editCluster}
                ...
                <div>
                  <label className="text-xs text-muted-foreground/50 mb-1 block">Hours</label>
                  <input
              ...
              <div>
                <label className="text-xs text-muted-foreground/50 mb-1 block">Topic</label>
                <input
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="done" className="text-sm text-foreground/80">Completed / done</label>
                <div className="ml-auto w-2.5 h-2.5 rounded-full opacity-70" style={{ backgroundColor: CLUSTER_COLORS[editCluster] }} />
              </div>
              <div className="flex justify-end gap-2 pt-1">
                <button onClick={() => setState('manual')}
                  className="text-xs text-muted-foreground/60 hover:text-foreground/80 px-3 py-1.5 rounded-lg border border-surface-border/40 hover:border-surface-border/60 transition-all">
                  Edit manually
                </button>
          ...
          {state === 'manual' && (
            <div className="space-y-3">
              {!llmOnline && (
                <p className="text-xs text-amber-400/70">AI offline \u2014 fill in manually</p>
              )}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-muted-foreground/50 mb-1 block">Cluster</label>
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="done2" checked={editDone}
                  onChange={e => setEditDone(e.target.checked)} className="accent-blue-500" />
                <label htmlFor="done2" className="text-sm text-foreground/80">Completed</label>
              </div>
